...
func ApplyTransactionWithResult(msg *core.Message, gp *core.GasPool, statedb state.StateDB, header *EvmHeader, tx *types.Transaction, usedGas *uint64, evm *vm.EVM) (*types.Receipt, *core.ExecutionResult, error) {
	// Create a new context to be used in the EVM environment.
	txContext := NewEVMTxContext(msg)
	evm.Reset(txContext, statedb)
